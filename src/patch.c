...
void my_exit();                     // changed from int to void
bool rev_in_string();
char *fetchname();
long atol();
long lseek();
...
static long p_base;                     /* where to _diff-type this time */
...
char *filename;
{
    ...
    /*NOSTRICT*/
    i_ptr = (char **)calloc((iline + 2), (MEM) sizeof(char *));
...
char *filename;
{
    ...
    tireclen = maxlen;
    tibuf[0] = calloc(1, (MEM)(BUFFERSIZE + 1));
    tibuf[1] = calloc(1, (MEM)(BUFFERSIZE + 1));
    if (tibuf[1] == Nullch)
...
void get_some_switches()
{
    ...
    while ((c = getopt_long(Argc, Argv, "-b:cd:D:elno:pr:Rsx:+", long_options, &option_index)) != -1) {
        switch (c) {
            case '+':
                return;
            case 'b':
...
another_hunk()
{
    ...
    if (diff_type == CONTEXT_DIFF) {
        ...
        while (p_end < p_max) {
            ...
            case ' ':
                context++;
                p_line[p_end] = savestr(buf+2);             // Calls this to savestr() into buf
                break;
            default:
